...
# Find layouts that contain the word "portrait" in their names
matching_layouts = []
...
for layout in p.listLayouts():
    if not layout.mapSeries is None and layout.mapSeries.enabled:
        ...
        arcpy.management.SelectLayerByAttribute(index_layer, "NEW_SELECTION", f"{index_layer_field} = 'PORTRAIT'")
...
# Export selected features for each "PORTRAIT" layout
