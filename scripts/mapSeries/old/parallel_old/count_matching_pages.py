...
def count_matching_pages():
    try:
        print(f"Opening project at {project_path}", file=sys.stderr)  # Debug output
        p = arcpy.mp.ArcGISProject(project_path)
        total_matching_pages = 0

        for layout in p.listLayouts():
            print(f"Checking layout: {layout.name}", file=sys.stderr)  # Debug output
...
if __name__ == "__main__":
    ...
    if total_pages >= 0:
        ...
        print("Error occurred while counting matching pages.", file=sys.stderr)
